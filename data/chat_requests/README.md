...
`kotenai-zeus-client` 2.3.x `load_for_turn` looks for `chat_request_{mode}_v2.json` under `{bucket}__{scope}` then the catalog root. Those copies are the same pin so a fresh clone can boot against Zeus 0.7.x (`client-floor-6.1`) before a live catalog sync.
