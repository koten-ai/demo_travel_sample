...
def extract_stamped_hash(doc: dict) -> str:
    ...
    def _is_real(h: str) -> bool:
        ...
        if "TO_BE_FILLED" in hs:
            return False
        return True
    # Prefer contract.hash from any candidate (doc, stamped_chat_request, stamped)
    # before falling back to _hash/hash — placeholders on the top-level doc must not
    # block a real hash inside a stamped wrapper.
    candidates = (doc, doc.get("stamped_chat_request") or {}, doc.get("stamped") or {})
